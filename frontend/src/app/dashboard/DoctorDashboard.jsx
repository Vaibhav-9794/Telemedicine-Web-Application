...
import {
  Calendar, FileText, Users, Check, X, MessageSquare, Plus, Trash2
} from 'lucide-react';

export default React.memo(function DoctorDashboard() {
  const { user } = useAuth();
  const { addToast } = useToast();

  // SWR cached fetching — instant on revisit
  const { data: appointments, isLoading: loading, mutate } = useAPI('/appointments/doctor');
  ...
  const handleStatusChange = useCallback(async (apptId, status) => {
    try {
      ...
      mutate(); // SWR revalidation — updates cached data
    } catch (error) {
  ...
  const handleOpenPrescribeModal = useCallback((appt) => {
    setActiveAppt(appt);
    setMedicines([]);
    setInstructions('');
  ...
  const handleRemoveMedicine = useCallback((idx) => {
    setMedicines(prev => prev.filter((_, i) => i !== idx));
  }, []);

  const handleSubmitPrescription = useCallback(async (e) => {
    e.preventDefault();
  ...
  }, [medicines, activeAppt, instructions, addToast, mutate]);
  ...
  const pendingCount = useMemo(() => apptList.filter(a => a.status === 'pending').length, [apptList]);
  const activeCount = useMemo(() => apptList.filter(a => a.status === 'accepted').length, [apptList]);
  const completedCount = useMemo(() => apptList.filter(a => a.status === 'completed').length, [apptList]);

  return (
    <div className="space-y-6 text-left relative">
      <div>
        <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Doctor Console</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="glass-card p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-black text-slate-850 dark:text-slate-105">{pendingCount}</p>
          </div>
        ...
        <div className="glass-card p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs text-slate-400 font-semibold">Accepted Consultations</p>
            <p className="text-2xl font-black text-slate-850 dark:text-slate-105">{activeCount}</p>
          </div>
        ...
        <div className="glass-card p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs text-slate-400 font-semibold">Completed Sessions</p>
            <p className="text-2xl font-black text-slate-850 dark:text-slate-105">{completedCount}</p>
          </div>
      ...
      <div className="glass-card p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-4">Patient Schedules Queue</h3>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {apptList.map((appt) => (
                  <tr key={appt._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
                    <td className="py-4">
                      <div className="font-bold text-slate-800 dark:text-slate-200">{appt.patientId?.name || 'N/A'}</div>
                    ...
                    <td className="py-4">
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                        appt.status === 'completed'
                          ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-900/30'
                          : appt.status === 'accepted'
                          ? 'bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 border border-teal-100/50 dark:border-teal-900/30'
                          : appt.status === 'rejected'
                          ? 'bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-455 border border-rose-100/50 dark:border-rose-900/30'
                          : 'bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 border border-amber-100/50 dark:border-amber-900/30'
                      }`}>
                        {appt.status}
                      </span>
                    </td>
                    <td className="py-4 text-center">
                      <div className="flex justify-center gap-2">
                        {appt.status === 'pending' && (
                          <>
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-teal-50 hover:bg-teal-100 dark:bg-teal-950/40 dark:hover:bg-teal-900/40 text-teal-600 dark:text-teal-400 transition-colors border border-teal-100/30 dark:border-teal-900/20"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/40 text-rose-600 dark:text-rose-400 transition-colors border border-rose-100/30 dark:border-rose-900/20"
                        ...
                        {appt.status === 'accepted' && (
                          <>
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-bold transition-all shadow-sm"
                            >
                              Prescribe Rx
                            </button>
                            <Link
                              href="/chat"
                              className="p-2 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-455 transition-colors border border-slate-200/50 dark:border-slate-750/30"
                              title="Send Message"
                            >
                        ...
                        {appt.status === 'completed' && (
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 italic">Consulted</span>
                        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 w-full max-w-xl rounded-3xl p-6 shadow-xl space-y-6 text-left max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700/40 pb-3">
              <div>
                <h4 className="text-sm font-bold text-slate-850 dark:text-slate-100">Write Medical Prescription</h4>
            ...
            <form onSubmit={handleSubmitPrescription} className="space-y-4">
              ...
              <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200/30 dark:border-slate-750/30">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-450">Instructions</label>
                    <input
                      type="text"
                ...
                <button
                  ...
                  className="w-full py-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-950/40 dark:hover:bg-primary-900/40 border border-primary-100/30 dark:border-primary-900/20 text-primary-600 dark:text-primary-400 font-bold rounded-xl text-xs transition-colors flex items-center justify-center gap-1"
                >
                  <Plus size={14} />
                  Add to Rx List
                </button>
              ...
              <div className="space-y-2">
                ...
                {medicines.length > 0 ? (
                  ...
                  <p className="text-[10px] text-slate-450 dark:text-slate-500 italic">No medicines added yet.</p>
                )}
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-205 dark:border-slate-750 rounded-xl text-xs sm:text-sm focus:outline-none resize-none"
                ></textarea>
              </div>
              ...
              <div className="flex gap-3 justify-end border-t border-slate-100 dark:border-slate-700/40 pt-4">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm"
                >
                  Authorize & Submit
                </button>
              </div>
